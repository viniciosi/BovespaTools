...
for indice, row in empresas.iloc[1801:,].iterrows():
    try:
        ep.main(str(row[0]) + "_" + str(row[1]) + "_CPRO_" + row[2].replace("/", ""), row[1], 4, row[0], 'Dados da Empresa', 'Proventos em Dinheiro', "PR14", row[3], row[4])
    except:
        print()
