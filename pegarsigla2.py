...
for indice, row in empresas.iloc[601:1201,].iterrows():
    ...
    log = open("log" + "VOL2", 'a', encoding = 'utf-8')
    ...
    try:
        ...
        if len(driver.find_elements_by_xpath("//a[@class='linkBusca']")) == 4 or 
                    row[2] == "GERDAU S.A.                                       " or
                    row[2] == "JEREISSATI PARTICIPACOES S.A.                     " or
                    row[2] == "CRISTAL PIGMENTOS DO BRASIL S.A.                  " or
                    row[2] == "NEOENERGIA S.A.                                   " or
                    row[2] == "GAFISA S.A.                                       " or
                    row[2] == "KROTON EDUCACIONAL S.A.                           " or
                    row[2] == "ITAU UNIBANCO HOLDING S.A.                        " or
                    row[2] == "BR MALLS PARTICIPACOES S.A.                       " or
                    row[2] == "BR PROPERTIES S.A.                                " or 
                    row[2] == "TRANSMISSORA ALIAN" or
                    row[2] == "TERRA SANTA AGRO S.A.                             " or
                    row[2] == "ALGAR TELECOM S" or
                    row[2] == "DURATEX S.A.                                      " or
                    row[2] == "ENEVA S.A                                         ":
            # segue

            sigla = driver.find_elements_by_xpath("//font[@face='Arial']")[8].text
